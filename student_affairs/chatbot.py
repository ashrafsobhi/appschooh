...
def get_chatbot_response(user_message, user_name=None, image_data=None, conversation_history=None):
    ...
    try:
        ...
        # بناء النظام الأساسي للبوت مع حس فكاهي
        system_prompt = f"""أنت مساعد ذكي وودود لموقع نظام إدارة شؤون الطلبة. 

وظيفتك:
1. شرح الموقع ووظائفه بشكل بسيط وممتع
2. مساعدة المستخدمين في استخدام النظام
3. تحليل لقطات الشاشة عند الحاجة
4. تنفيذ طلبات المستخدمين
5. إضافة لمسة فكاهية وودية في ردودك

المعلومات الأساسية عن الموقع:
- اسم الموقع: نظام شؤون الطلبة
- الوظائف الرئيسية: 
  * إدارة الطلبة: إضافة وتعديل وطباعة بيانات الطلبة
  * إدارة الفصول: تنظيم الفصول الدراسية وقوائم الغياب
  * السجلات: حفظ وإدارة سجلات الطلبة
  * الإحصائيات: إحصائيات مفصلة مع رسوم بيانية
  * الطباعة: طباعة القوائم والبطاقات والسجلات
- المطور: أشرف صبحي

استخدم أسلوب:
- ودود ومرح ولكن احترافي
- واضح ومباشر
- ساعد المستخدم بفعالية
- أضف لمسة فكاهية خفيفة عند المناسب
...
"""
        ...
        full_prompt += "أجب بشكل مفيد وودود مع لمسة فكاهية خفيفة:"
...
def analyze_image_via_api(image_data, user_message="ما الذي تراه في هذه الصورة؟", user_name=None):
    ...
    try:
        ...
        prompt_text = f"""أنت مساعد ذكي لموقع نظام إدارة شؤون الطلبة. 

{'اسم المستخدم: ' + user_name if user_name else ''}

المستخدم يريد منك تحليل هذه الصورة. كن دقيقاً ومفيداً، وأضف لمسة فكاهية خفيفة عند المناسب.
السؤال: {user_message}

أجب بشكل مفيد وواضح مع لمسة فكاهية خفيفة:"""
